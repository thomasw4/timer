...
void execute(int number, int timers, vector<int> timeDifference, string events[])
{
	...
	cout << "Time to start the " << events[number+1] << endl;
	}
	if(number < timers-2){
	
   	
   	cout << "In " << timeDifference.at(number+1) << " minutes, you need to start the " << events[number+2] << endl;
	}
	
	if(number == timers-2 ){
		
		sleep(timeDifference.at(timers));
		cout << "All times are complete" << endl;
...
void checkCommandLine(int numberOfTimers, int argc, int switchCase)
{
	if(argc != 3 || (switchCase < 1 || switchCase >3))
	{
	  cout << "To use this application: Enter ./timer [timer option] [# of timers]" << endl;
	  cout << "-------------------------------------------------------------------" << endl;
	  cout << endl;
	  cout << "timer options:" << endl;
	  cout << "1 for console timer" << endl;
	  cout << "2 for GUI timer" << endl;
	  cout << "3 for console input and GUI output" << endl;
	  exit(EXIT_FAILURE);
    } 
